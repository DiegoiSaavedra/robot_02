...
// Motor 1 (usa los mismos que tenías)
...
// Motor 2 (ajusta estos pines según tu hardware)
...
// ---------- Constantes y variables de control PID --1:100 2:60  1:0 2:0  --------
...
// Obtener posición en grados para Motor 1
float getPos1() {
  return (encoderCount1 * 360.0) / COUNTS_PER_REVOLUTION;
}

// Obtener posición en grados para Motor 2
float getPos2() {
...
// Mover Motor 1 hacia el ángulo deseado
...
// Mover Motor 2 hacia el ángulo deseado
...
// ---------- Loop principal ----------
void loop() {
  // Procesar comandos recibidos por el Monitor Serie
  if (Serial.available() > 0) {
    String inputString = Serial.readStringUntil('\n');
    inputString.trim();
    
    // Comandos especiales:
    // 's' para detener ambos motores
    // 'r' para reiniciar los contadores de los encoders
    if (inputString.equalsIgnoreCase("s")) {
      ...
      // Se espera el formato "1:150 2:90" (pueden estar separados por espacio o sin ellos)
  ...
  // Depuración: imprimir estado de cada motor cada DEBUG_INTERVAL
